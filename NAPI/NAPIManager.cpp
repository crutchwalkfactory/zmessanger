...
QStringList NAPIManager::getProfiles() 
{
	...
	for(int i = 0; i < profiles; i++, prof_ptr = buf + i * NAPI_MAX_PROFILE_NAME_LENGTH) {
		ret.append(QString::fromUtf8(prof_ptr));
	}
...
NAPILink *NAPIManager::openLink(QString profile) 
{
	if(profile.isEmpty())
	{
		QStringList list = getProfiles();
		ZSingleSelectDlg *dlg = new ZSingleSelectDlg("Internet", "", NULL);
		dlg->addItemsList(list);
		if ( dlg->exec() != QDialog::Accepted )
			return 0;

		profile =  *(list.at(dlg->getCheckedItemIndex()));
		delete dlg;
	}
