...
bool FlapPacket::send_to(int sock, int timeout)
{
 ...
 if ((t_network_error=t_send(sock, &tmp_buf[0], 6, MSG_NOSIGNAL, timeout))!=6) return false;
 if ((t_network_error=t_send(sock, &payload[0], payload.size(), MSG_NOSIGNAL, timeout))!=(int)payload.size()) return false;
 logMes_4("FlapPacket::send_to - end");
 return true;
}
...
{
 logMes_4("SignOnPacket::decode_from");
 if (v.size()<(start_ind+4)) return false;
...
{
 logMes_4("SignOnPacket::encode_to");
...
{
 logMes_4("BaseMsg::decode_header");
...
{
 logMes_4("BaseMsg::encode_header");
...
{
 logMes_4("BaseOutgoingMsg::encode_to");
...
{
 logMes_4("BaseIncomingMsg::decode_from");
...
{
 logMes_4("SimpleMsg::decode_from");
...
{
 logMes_4("RendezVousMsg::decode_from");
 size_t pos=0;
...
{
 logMes_4("RendezVousMsg::encode_to");
...
#define UNPACK_LE(arg) \
 ...
 if (msg_type==0x01) // Plain message optional data
  ...
  {
  	logMes_4("Decode xTraZ");
  uint16_t plug_info_len;
  UNPACK_LE(plug_info_len)

  if (data.size()<(pos+16)) 
  {
  	logMes_4("%d<%d - do not",data.size(),(pos+16));
  	return false;
	}
  ...
  if (memcmp(plug_guid, xtraz_script_guid, 16)!=0)
  {
  	logMes_4("I can't decode anything except xTraZ notifies");
   return false; // I can't decode anything except xTraZ notifies
	}
  
  UNPACK_LE(plug_func_id)
  if (plug_func_id!=0x08) 
  {
  	logMes_4("Script notify [plug_func_id=%d]",plug_func_id);
  	return false; // Script notify
  }
  uint32_t plug_name_len;
  UNPACK_LE(plug_name_len)
  if (data.size()<(pos+plug_name_len))
  {
  	logMes_4("data.size()<(pos+plug_name_len) - dont");
   return false;
  }
  ...
  if (data.size()<(pos+plug_msg_len)) 
  {
  	logMes_4("Warning: Data sive < pos + msg_len");
 	 //return false;
 } 

  xtraz_script_text.assign((const char *)&data[pos], plug_msg_len);
  //qxtraz_script_text = QString::fromUtf8((const char *)&data[pos], plug_msg_len);
...
{
  logMes_4("TLV2711::parseXtrazRequest");
...
/*
<NR><RES>&lt;ret event='OnRemoteNotification'&gt;&lt;srv&gt;&lt;id&gt;cAwaySrv&lt;/id&gt;&lt;val srv_id='cAwaySrv'&gt;&lt;Root&gt;&lt;CASXtraSetAwayMessage&gt;&lt;/CASXtraSetAwayMessage&gt;&lt;uin&gt;452035352&lt;/uin&gt;&lt;index&gt;1&lt;/index&gt;&lt;title&gt;


<NR><RES>&lt;ret event='OnRemoteNotification'&gt;&lt;srv&gt;&lt;id&gt;cAwaySrv&lt;/id&gt;&lt;val srv_id='cAwaySrv'&gt;&lt;Root&gt;&lt;CASXtraSetAwayMessage&gt;&lt;/CASXtraSetAwayMessage&gt;&lt;uin&gt;3927345&lt;/uin&gt;&lt;index&gt;3&lt;/index&gt;&lt;title&gt;
*/

// ----------------=========ooooOOOOOOOOOoooo=========----------------
bool TLV2711::parseXtrazResponse(void)
{
	//logMes_4("Text="+QString::fromUtf8(xtraz_script_text.c_str() ));
	//logMes_4("lenstr1=%d\nenstr2=%d", strlen(xtraz_script_text.c_str()), qxtraz_script_text.length());
	//if (xtraz_script_text.find("<NR>")!=xtraz_script_text.npos)
	//if (xtraz_script_text.find("</NR>")!=xtraz_script_text.npos) 
	if (xtraz_script_text.find("<RES>")==xtraz_script_text.npos)
	{
		logMes_4("Not pos <RES>");
		return false;
	}
	/*
	if (xtraz_script_text.find("</RES>")==xtraz_script_text.npos) 
	{
		logMes_4("Not pos </RES>");
		if (xtraz_script_text.find("</RE")!=xtraz_script_text.npos)
			logMes_4("QString rulit!!!");
		return false;
	}
	*/	
	...
	if (start_i!=xtraz_script_text.npos && end_i!=xtraz_script_text.npos) //return false;
	{
		start_i+=13;
		//if (end_i==xtraz_script_text.npos) return false;
...
{
 logMes_4("ExtendedMsg::decode_from");
 TLVPack tp;
 tp.data=tlv_v;
...
{
 logMes_4("ExtendedMsg::encode_to");
 tlv_v.clear();
...
{
 logMes_4("OldStyleMsg::OldStyleMsg");
...
{
 logMes_4("OldStyleMsg::decode_from");
 for (unsigned int i=0; i<tlv_v.size(); ++i)
...
{
 logMes_4("OldStyleMsg::encode_to");
 TLVField tlv;
 tlv.type=0x0005;
...
{
 logMes_4("SSIContactListItem::decode_from");
...
{
 logMes_4("SSIContactListItem::encode_to");
...
{
 logMes_4("SSIContactList::decode_from");
 if (v.size()<(start_ind+1))
 {
 	 logMes_4(">>>>>>>>>>>>1>v.size()=%d",v.size());
  return false;
 }
 ...
 if (v.size()<(start_ind+2))
 { 
 	logMes_4(">>>>>>>>>>>>2>v.size()=%d",v.size());
 	return false;
 }
 ...
 for (int i=0; i<items_count; ++i)
  ...
  if (!clit.decode_from(v, start_ind))
  {
  	logMes_4(">>>>>>>>>>>>>clit.decode_from");
    return false;
  }
 ...
 if (v.size()<(start_ind+4))
 {
   logMes_4(">>>>>>>>>>>>3>v.size()=%d",v.size());
   return false;
 }
...
{
 logMes_4("ICBMParameters::decode_from");
...
{
 logMes_4("ICBMParameters::encode_to");
