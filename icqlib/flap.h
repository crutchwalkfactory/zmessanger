...
class FlapPacket{
 public:
  ...
  bool recv_from(int sock, int timeout=10);
  bool send_to(int sock, int timeout=100);
...
class TLVField{
 public:
  TLVField(void);
  inline TLVField(uint8_t i, uint16_t t) { data.resize(1); data[0]=i; type=t; }
  inline TLVField(uint16_t i, uint16_t t) { putInt16(i); type=t; }
  inline TLVField(uint32_t i, uint16_t t) { putInt32(i); type=t; }
  inline TLVField(string s, uint16_t t) { putString(s); type=t; }
  inline TLVField(vector<uint8_t> & v, uint16_t t) { data=v; type=t; }
  inline TLVField(const uint8_t * p, size_t p_len, uint16_t t) { if (p_len>0 && p!=NULL) data.assign(p, p+p_len); else data.clear(); type=t; }
  ~TLVField();
