...
int t_connect(int sockfd, const struct sockaddr *serv_addr, socklen_t addrlen, int * timeout)
{
 ...
 logMes_4("Make socket non-blocked");
...
logMes_4(" Make poll in timeout currency");
 fd_set wfds;
 struct timeval tv;
...
// Make poll in timeout currency
 ...
 do
  ...
  FD_SET(sockfd, &wfds);
  sel_ret = select(sockfd+1, NULL, &wfds, NULL, &tv);
  //sel_ret = select(1, NULL, &wfds, NULL, &tv);
#ifndef _WIN32
  } while (sel_ret<0 && errno==EINTR);
#else
...
// Make poll in timeout currency
  ...
  //int sel_ret = select(1, NULL, &wfds, NULL, &tv);
  if (sel_ret<0)
   {
   #ifndef _WIN32
...
// Make poll in timeout currency
  ...
  //int sel_ret = select(50, &rfds, NULL, NULL, &tv);
  if (sel_ret<0)
   {
   #ifndef _WIN32
   if (errno==EINTR) // Interrupted by signal
  ...
  if (!FD_ISSET(s, &rfds))
   return TNETWORK_ERR;
  int l=recv(s, (unsigned char *)buf+(len-curr_len), curr_len, flags);
  if (l<0)
   {
   #ifndef _WIN32
   if (errno==EINTR) // Interrupted by signal
